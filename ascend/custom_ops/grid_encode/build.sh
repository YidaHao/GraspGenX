...
if [[ ! -f "$project/CMakeLists.txt" ]]; then
    "$PYTHON_BIN" "$generator" gen -i "$root/ops.json" -op GridEncode \
...
cmake -G "Unix Makefiles" -S "$project" -B "$root/build/cmake_opp" \
    ...
    -DASCEND_COMPUTE_UNIT=ascend310p -Dvendor_name=graspgenx_grid \
    -DASCEND_PYTHON_EXECUTABLE="$PYTHON_BIN" \
    -DENABLE_SOURCE_PACKAGE=ON -DENABLE_BINARY_PACKAGE=ON -DENABLE_TEST=OFF
# CANN 8.1 can leave this temporary directory after failed compilation.
rm -rf "$root/build/cmake_opp/op_kernel/GridEncode_ascend310p"
...
${CXX:-c++} -O2 -std=c++17 -fPIC -shared "$root/torch_bridge.cpp" \
    ...
    -DTORCH_EXTENSION_NAME=grid_encode_bridge \
