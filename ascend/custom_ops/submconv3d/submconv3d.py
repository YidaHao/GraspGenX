...
Coordinates are unique int32 [batch, x, y, z] rows (negative xyz is valid).
A neighbor map belongs to the exact coordinate values AND row order that built
it. Uniqueness and map contents are caller preconditions, not host-side scans.
...
def _check_indices(indices, kernel_size):
...
def _build_subm_map_fake(indices, kernel_size):
...
def _build_subm_map_ge(indices, kernel_size, meta_outputs=None):
    return ge.custom_op(
        "BuildSubmMap",
...
def build_subm_map(indices, kernel_size=3):
    ...
    """
    _check_indices(indices, kernel_size)
    return torch.ops.graspgenx_subm.build_subm_map(indices, kernel_size)
